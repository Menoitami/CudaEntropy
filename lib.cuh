                                           ...
                                           double *bins_global)
{
    ...
    // Массивы для хранения параметров системы и ее положение
    // Массивы выделяются на локальной памяти, из-за более высокой скорости работы при таком выделении памяти
    // Ускорение в ~5 раз
    // Максимальный размер массивов может быть ~1024 в сумме, иначе нужно переносить их в глобальную память
    // !В случае если парамтров или координат системы больше чем размер этих массивов (32), нужно изменить размеры под актуальное разрешение!
    double X_locals[32];
